  ...
  return (
    <>
      ...
      <form onSubmit={submit} className="mt-6 grid gap-4" noValidate>
        <Field label="Email" name="email" type="email" autoComplete="email" inputMode="email" required value={email} onChange={(e) => setEmail(e.target.value)} />
        ...
        <ul className="mt-2 grid gap-1.5">
          {DEMO.map(([role, e, p]) => (
            <li key={e}>
              ...
              >
                <span className="font-semibold">{role}</span> <span className="text-muted-foreground">{e}</span>
              </button>
            </li>
          ))}
        </ul>
